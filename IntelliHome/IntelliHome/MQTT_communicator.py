...
class MQTT_communicator:
    ...
    def on_mqtt_connect(self, client, userdata, flags, rc, properties=None):
        """Callback for when MQTT client connects - V2 Signature (5 positional args + self)"""
        if rc == 0:
    ...
    def on_mqtt_disconnect(self, client, userdata, rc, properties=None):
        """Callback for when MQTT client disconnects - V2 Signature (4 positional args + self)"""
        self.mqtt_connected = False
    ...
    def on_mqtt_publish(self, client, userdata, mid, reasonCode, properties):
        """Callback for when message is published - V2 Signature (5 positional args + self)"""
        if reasonCode == 0:
    ...
    def subscribe_to_feeds(self, feeds):
        ...
        for feed_key in feeds:
            ...
            # Subscribe using QoS 1 for control messages (as recommended in project docs)
